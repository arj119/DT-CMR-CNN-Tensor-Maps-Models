...
# load numpy arrays
def load_npz_files(input_list, output_list, output_channels, scale_factor=1):
    ...
    for idx, val in enumerate(input_list):
        if idx == 0:
            input_arrays = load_data_into_array(val, scale_factor)
        else:
            temp = load_data_into_array(val, scale_factor)
            input_arrays = np.concatenate((input_arrays, temp), axis=0)

    for idx, val in enumerate(output_list):
        if idx == 0:
            output_arrays = load_data_into_array(val, scale_factor, is_input=False)
        else:
            temp = load_data_into_array(val, scale_factor, is_input=False)
...
def random_crop(img_in, img_out, height, width, input_channels, output_channels):
    ...
    r_img_in = cropped_image[:, :, :input_channels]
...
def augment(img_in, img_out, height, width, input_channels, output_channels):
    ...
    img_in, img_out = random_crop(img_in, img_out, height, width, input_channels, output_channels)
